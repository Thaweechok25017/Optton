...
static void fallingEdge(void) {
	...
	static void send4Bits(char data) {
		HAL_GPIO_WritePin(D4_GPIO_PORT, D4_Pin, SET_IF(data&0x01));
		HAL_GPIO_WritePin(D5_GPIO_PORT, D5_Pin, SET_IF(data&0x02));
		HAL_GPIO_WritePin(D6_GPIO_PORT, D6_Pin, SET_IF(data&0x04));
		HAL_GPIO_WritePin(D7_GPIO_PORT, D7_Pin, SET_IF(data&0x08));
