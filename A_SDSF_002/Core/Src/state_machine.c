...
void stateCustomTask() {
	...
	while (customRunning) {
		...
		case OPEN_BEGIN:
			...
			break;
		case OPEN_SELECT:
			Oled_ShowMessage_sm("Choose OPEN?", 0, 0);
			Oled_ShowMessage_sm("Init(1)  Custom(3)", 0, 53);
		...
		case OPEN_CUSTOM:
			...
			break;
		case CLOSE_BEGIN:
			Oled_ShowMessage_sm("Modify CLOSE?", 0, 0);
			Oled_ShowMessage_sm("Yes(1)       No(3)", 0, 53);
			...
			break;
		case CLOSE_SELECT:
			Oled_ShowMessage_sm("Choose CLOSE?", 0, 0);
			Oled_ShowMessage_sm("Init(1)  Custom(3)", 0, 53);
		...
		case CLOSE_CUSTOM:
			...
			break;
		case DONE:
			Oled_ShowMessage_sm("Successful", 0, 0);
			HAL_Delay(1000);
			Oled_Clear();
			customState = SETTING;
...
void setCustomPattern(PatternType patrn) {
	...
	strcpy(msg, ">");
	Oled_ShowMessage_sm(msg, 0, 23);

	if (patrn == PATTERN_OPEN) {
		Oled_ShowMessage_sm("Customize OPEN", 0, 0);
		pPattern[0] = &openPattern[0];
		pPattern[1] = &openPattern[1];
		pPattern[2] = &openPattern[2];

	} else if (patrn == PATTERN_CLOSE) {
		Oled_ShowMessage_sm("Customize CLOSE", 0, 0);
	...
	while (isProcess) {
		...
		if (*pPattern[0] != 0 && *pPattern[1] != 0 && *pPattern[2] != 0) {
			if (patrn == PATTERN_OPEN && closePattern[0] == *pPattern[0] && closePattern[1] == *pPattern[1] && closePattern[1] == *pPattern[1]) {
				Oled_ShowMessage_sm("same Close pattern", 0, 35);
